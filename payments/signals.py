...
def admin_payments_updated(**kwargs):
    ...
    TaxesPerMonth.objects.filter(pk=1).update(
        **{
            str(curr_month): curr_tax
            for curr_month in month_keys[curr_month : len(month_keys)]
        }
    )
    SalariesPerMonth.objects.filter(pk=1).update(
        **{
            str(curr_month): curr_salaries
            for curr_month in month_keys[curr_month : len(month_keys)]
        }
    )
...
def individual_taxes_payed(instance, **kwargs):
    if IndividualPayment.objects.filter(id=instance.id):
        ...
        taxes_needed_per_month = {
            curr_field.name: curr_field.value_from_object(tax_object)
            for curr_field in tax_object._meta.get_fields()
            if curr_field.name in month_keys
        }
        months_payed = [
            curr_field.name
            for curr_field in form._meta.get_fields()
            if curr_field.name != "id"
            and curr_field.value_from_object(instance) == True
            and curr_field.value_from_object(form) == False
        ]
        IndividualTaxesPayed.objects.filter(pk=instance.id).update(
            **{
                str(curr_month): taxes_needed_per_month[curr_month]
                for curr_month in months_payed
            }
        )
...
def get_available_payments(**kwargs):
    if ("args" in kwargs and len(kwargs["args"]) > 0) and (
        TaxesPerMonth.objects.exists()
    ):
        ...
        taxes_payed_per_month = {
            curr_field.name: curr_field.value_from_object(payed_object)
            for curr_field in payed_object._meta.get_fields()
            if curr_field.name in month_keys
        }
        taxes_needed_per_month = {
            curr_field.name: curr_field.value_from_object(tax_object)
        ...
        IndividualPayment.objects.filter(pk=curr_id).update(
            **{
                str(curr_month): taxes_needed_per_month[curr_month]
                <= taxes_payed_per_month[curr_month]
                for curr_month in month_keys
            }
        )
...
def salaries_payed(instance, **kwargs):
    if SalariesPayment.objects.exists():
        ...
        salaries_needed_per_month = {
            curr_field.name: curr_field.value_from_object(salaries_object)
            for curr_field in salaries_object._meta.get_fields()
            if curr_field.name in month_keys
        }
        months_payed = [
            curr_field.name
        ...
        SalariesPayedPerMonth.objects.filter(pk=1).update(
            **{
                str(curr_month): salaries_needed_per_month[curr_month]
                for curr_month in months_payed
            }
        )
...
def get_available_salaries(**kwargs):
    if SalariesPayedPerMonth.objects.exists():
        ...
        SalariesPayment.objects.filter(pk=1).update(
            **{
                str(curr_month): salaries_needed_per_month[curr_month]
                <= slaries_payed_per_month[curr_month]
                for curr_month in month_keys
            }
        )
...
def create_news_for_tax_or_salaries_change(instance, **kwargs):
    if PaymentsAdmin.objects.exists():
        ...
        if new_tax != previous_tax:
            notification = News(
                title="Tax changed",
                content=f"The tax has changed from {previous_tax} to {new_tax}",
            )
            notification.save()
        if new_salary != previous_salary:
            notification = News(
                title="Salaries changed",
                content=f"The salaries have changed from {previous_salary} to {new_salary}",
            )
            notification.save()
