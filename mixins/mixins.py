...
class HouseholdAdminRequiredMixin(AccessMixin):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        is_hh_admin = (
            ...
            == UserProfile.objects.get(pk=request.user.id).apartment
        )
...
class HouseholdAdminOfUserRequiredMixin(AccessMixin):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        is_hh_admin = (
            UserProfile.objects.get(pk=request.resolver_match.kwargs["pk"]).apartment
            == UserProfile.objects.get(pk=request.user.id).apartment
        ) and (UserProfile.objects.get(pk=request.user.id).is_household_admin)
        if not (is_hh_admin or request.user.is_superuser):
            return self.handle_no_permission()
        return super().dispatch(request, *args, **kwargs)
