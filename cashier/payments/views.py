...
from cashier.mixins.mixins import (
    OwnerOrHouseholdAdminOrSuperUserRequiredMixin,
...
class PaySalaries(SuperUserRequiredMixin, BootStrapFormMixin, UpdateView):
    ...
    def get_form_class(self):
        ...
        self.fields = [
            str(curr_field.name)
            for curr_field in self.model._meta.get_fields()
            if not curr_field.value_from_object(self.model.objects.get(pk=1))
        ]
    ...
    def get_context_data(self, **kwargs):
        ...
        tax_info = [
            (
                curr_field.name,
                curr_field.value_from_object(tax_object),
                taxes_payed_per_month[curr_field.name],
            )
        ...
        if "total_taxes_needed" not in kwargs:
            kwargs["total_taxes_needed"] = sum(
                [curr_value[1] for curr_value in tax_info]
            )
...
class PaymentTypes(LoginRequiredMixin, View):
    def get(self, request):
        is_hh_admin = UserProfile.objects.get(
        ...
        return render(
            ...
            template_name="payments/view_payments.html",
            context=context,
        )


class MakePaymentView(
    OwnerOrHouseholdAdminOrSuperUserRequiredMixin, BootStrapFormMixin, UpdateView
):
    ...
    def get_form_class(self):
        ...
        self.fields = [
            str(curr_field.name)
            for curr_field in self.model._meta.get_fields()
            if not (curr_field.value_from_object(self.object))
        ]
        return model_forms.modelform_factory(self.model, fields=self.fields)
    ...
    def get_success_url(self):
        return reverse_lazy(
            "make_payment", kwargs={"pk": self.kwargs.get(self.pk_url_kwarg)}
        )
