...
class DeleteProfileViewTest(CashierTestCase):
    ...
    def test_profile_owner(self):
        response = self.client.get(reverse('delete_profile', kwargs={'pk': self.user.id}))
        self.assertTrue(response.context['profile_owner'])

    def test_user_deactivation__profile_apartment_and_household_update(self):
        asd = self.user
        test_repsonse = self.client.post(reverse('delete_profile', kwargs={'pk': self.user.id}), data={'result' : 'Delete'})
        aaa = test_repsonse
