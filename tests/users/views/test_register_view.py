...
class RegisterViewTest(CashierTestCase):
    def test_both_user_and_profile_forms_are_loaded(self):
        ...
        self.assertTrue(
            response.context["profile_form"].__class__.__name__ == "EditProfileForm"
        )

    def test_user_form_fields_are_correct(self):
        response = self.client.get(reverse("register_view"))
        self.assertListEqual(
            list(response.context["user_form"].fields.keys()),
            ["username", "password1", "password2"],
        )

    def test_profile_form_fields_are_correct(self):
        response = self.client.get(reverse("register_view"))
        profile_form_shown_fields = [
            "first_name",
            "last_name",
            "email",
            "phone_number",
        ...
        self.assertListEqual(
            list(response.context["profile_form"].fields.keys()),
            profile_form_shown_fields,
        )
    ...
    def test_user_and_profile_creation(self):
        ...
        self.assertEqual(
            self.profile.newsletter_agreement, self.test_newsletter_agreement
        )
