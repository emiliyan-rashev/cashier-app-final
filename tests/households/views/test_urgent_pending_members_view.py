...
class UrgentPendingMembersTest(CashierTestCase):
    ...
    def test_user_with_hh_admin_not_appearing(self):
        hh_profile = HouseholdProfile(pk=self.profile.apartment, apartment=self.profile.apartment)
        hh_profile.save()
        profile = self.profile
        profile.household = hh_profile
        profile.is_household_admin = True
        profile.save()
        self.create_user(test_username='test_2', test_email='test2_email@test.qwe', test_apartment=1)
        self.client.force_login(self.super_user)
        response = self.client.get(reverse('urgent_pending_members'))
    ...
    def test_two_users_in_the_same_hh_without_having_hh_admin(self):
        self.create_user(test_username='test_3', test_email='test3_email@test.qwe', test_apartment=1)
        self.client.force_login(self.super_user)
        response = self.client.get(reverse('urgent_pending_members'))
