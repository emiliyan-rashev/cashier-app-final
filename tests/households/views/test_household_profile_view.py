...
class HouseholdProfileTest(CashierTestCase):
    def setUp(self) -> None:
        self.client = Client()
        self.create_user(test_username='test1_username', test_email='test1_email@test.qwe', test_apartment=1)
        self.create_user(test_username='test2_username', test_email='test2_email@test.qwe', test_apartment=1)
        self.create_user(test_username='test3_username', test_email='test3_email@test.qwe', test_apartment=1)
        self.create_user(test_username='test4_username', test_email='test4_email@test.qwe', test_apartment=1)
        self.create_user(test_username='test5_username', test_email='test5_email@test.qwe', test_apartment=2)
    ...
    def get_response(self):
        self.response = self.client.get(reverse('hh_profile', kwargs={'pk': self.hh_profile.apartment}))
    ...
    def test_not_approved_members(self):
        ...
        self.assertListEqual(
            ...
            list(UserProfile.objects.filter(user__in=[self.user2, self.user3, self.user4]).order_by('user'))
        )
        self.deactivate_user(self.user2)
        self.assertListEqual(
            list(self.response.context['not_approved_members'].order_by('user')),
            list(UserProfile.objects.filter(user__in=[self.user3, self.user4]).order_by('user'))
        )
    ...
    def test_not_living_in_hh(self):
        ...
        self.assertListEqual(
            ...
            list(UserProfile.objects.filter(user__in=[self.user1, self.user2, self.user3, self.user4]).order_by('user'))
        )
