...
class DeleteNewsTest(CashierTestCase):
    ...
    def test_get_without_any_comments(self):
        response = self.client.get(reverse('comment_news_view', kwargs={'pk': self.news.id}))
    ...
    def test_get_with_comments(self):
        ...
        response = self.client.get(reverse('comment_news_view', kwargs={'pk': self.news.id}))
        self.assertListEqual(list(response.context['all_comments']), list(Comment.objects.filter(content=self.content, News=self.news, user=self.user)))

    def test_post(self):
        self.client.post(reverse('comment_news_view', kwargs={'pk': self.news.id}), data={'content': self.content})
        response = self.client.get(reverse('comment_news_view', kwargs={'pk': self.news.id}))
        self.assertListEqual(list(response.context['all_comments']), list(Comment.objects.filter(content=self.content, News=self.news, user=self.user)))